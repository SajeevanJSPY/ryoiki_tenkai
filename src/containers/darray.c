...
void* _darray_create(u64 length, u64 stride, frame_allocator_int* allocator) {
    ...
    if (allocator) {
        ...
        new_array = rallocate(header_size + array_size, MEMORY_TAG_DARRAY);
    }
    rset_memory(new_array, 0, header_size + array_size);
    if (length == 0) {
        RFATAL("_darray_create called with length of 0");
    }
...
void darray_destroy(void* array) {
    if (array) {
        ...
        if (header->allocator) {
            ...
            rfree(header, total_size, MEMORY_TAG_DARRAY);
        }
    }
}
...
void* _darray_resize(void* array) {
    ...
    if (header->capacity == 0) {
        RFATAL("_darray_resize called on an array with 0 capacity. This should not be possible.");
        return 0;
    }
    ...
    rcopy_memory(temp, array, header->length * header->stride);
...
void* _darray_push(void* array, const void* value_ptr) {
    ...
    rcopy_memory((void*)addr, value_ptr, header->stride);
...
void _darray_pop(void* array, void* dest) {
    ...
    if (length < 1) {
        RERROR("darray_pop called on an empty darray. Nothing to be done.");
        return;
    }
    u64 addr = (u64)array;
    addr += ((length - 1) * stride);
    rcopy_memory(dest, (void*)addr, stride);
...
void* darray_pop_at(void* array, u64 index, void* dest) {
    ...
    u64 stride = darray_stride(array);
    if (index >= length) {
        RERROR("Index outside the bounds of this array! Length: %i, index: %index", length, index);
        return array;
    }

    u64 addr = (u64)array;
    rcopy_memory(dest, (void*)(addr + (index * stride)), stride);
...
void* _darray_insert_at(void* array, u64 index, void* value_ptr) {
    ...
    rcopy_memory((void*)(addr + (index * stride)), value_ptr, stride);
